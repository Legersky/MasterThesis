...
Only expanding bases were considered as it is a necessary condition for convergence of the extending window method. At the same time, convergence of Phase~1 is guaranteed.
...
We made the following remarks:
\begin{enumerate}
\item See for instance Examples \ref{ex:integerAJ} and \ref{ex:integerAK} -- congruence classes modulo $\beta-1$ are different, since we work in $\ZZ\left[\frac{1}{2} \sqrt{5} + \frac{1}{2}\right]$ for the former and in $\ZZ\left[-\sqrt{5} + 1\right]$ for the latter one.
...
\item If an alphabet is minimal, there is only one congruence class modulo $\beta$ which has two representatives in the alphabet, i.e, it is only litle  redundant.
...
\item Examples \ref{ex:integerAB} and \ref{ex:integerAO} show that  an alphabet with one more digit than the minimal one has shorter length of window.
