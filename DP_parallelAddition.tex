...
C. Frougny, E. Pelantov\'a and M. Svobodov\'a provided parallel addition algorithms with an integer alphabet for all bases $\beta$ such that $|\beta|>1$ and no conjugate of $\beta$ equals 1 in modulus, see \cite{parAddNS}. Nevertheless, the alphabet is not minimal in general.
 The parallel addition algorithms for several bases (negative integers, complex numbers $\imath-1, 2\imath$ and $\sqrt{2}\imath$, quadratic Pisot unit and  non-integer rationals) with minimal integer alphabet are presented in \cite{minAlph}.
 
We are interested in construction of parallel addition algorithm for a given base $\beta$ which is an algebraic integer such that $|\beta>1|$ and possibly non-integer alphabet $\A$. Our aim is to implement method which attempts to construct an algorithm automatically for the numeration system $(\beta,\A)$. 
...
Finally, several examples of results can be found in Chapter~\ref{chap:testing}. We compare various methods from Chapter~\ref{chap:diffChoices}. We discuss successful numeration systems with a quadratic base and integer or non-integer alphabet.
