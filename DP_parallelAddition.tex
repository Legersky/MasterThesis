...
We  recalled necessary definitions and notation. The concept of parallel addition and extending  window method was explained. We proposed various methods for both phase. Some of them involve $\beta$-norm, which was constructed by using the companion matrix of the minimal polynomial $m_\beta$.

We solved convergence of Phase~1 as a sufficient condition is that the base $\beta$ is expanding, i.e., all its conjugates are greater than one in modulus. We showed that it is also a necessary condition of convergence of the whole extending method. 

The check convergence of Phase~2 for $bb\dots b$ inputs was reviewed. Next, we introduced the notion of stable Phase~2 and a Rauzy graph. We proved Theorem~\ref{thm:infinitePathInRauzyGraph} which may reveal non-convergence of Phase~2 by searching for a cycle in the Rauzy graph.

We indicated that there is the same lower bound on the size of an alphabet from $\Zbeta$ as of an integer one.

Algorithms based on the obtained theoretical results were design. Algorithm~\ref{alg:weightFunction_modified} summarizes Phase~2 with the control of non-convergence. The extending window method was implemented with all proposed algorithms.
