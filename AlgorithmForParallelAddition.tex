...
Return $(\beta,\A)$-representation of the number represented by the list \var{\_w} of digits from the input alphabet $\B$. According to the equation \eqref{eq:conversionFormula}, it is computed locally by using the weight function $q$. An exception is raised when an outputted digit is not in the alphabet $\A$.
\end{method}
